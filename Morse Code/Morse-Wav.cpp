...
* Convert morse code to MONO or STEREO Audio WAV file using PCM
*
...
#define EPW 50      // elements per word (definition)
    ...
    long wav_size;  // wave size

public:
    /**
    * Constructor
    */
    MorseWav(const char* morsecode, const char* filename, double tone, double wpm, double samples_per_second, bool play, int modus)
    {
        Path = filename;
        MorseCode = morsecode;
        MONO_STEREO = modus;
        ...
        */
        if (play)
        {
            string str = "start \"\" "; // auto closes the 'starter' cmd promt
            str += Path;
            str += " /play /close";
            str += Path;
